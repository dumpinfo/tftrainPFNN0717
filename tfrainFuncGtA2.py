...
def PFNNet(X_nn, keep_prob, reuse):
  with tf.variable_scope('PFNNet', reuse = reuse):
     ####parameter of nn                                                                                           
...
reuse_vars = False
ii = 0
_xx = X_nn[ii * batch_size: (ii+1) * batch_size]
_yy = Y_nn[ii * batch_size: (ii+1) * batch_size]

rng = np.random.RandomState(23456)   
keep_prob = tf.placeholder(tf.float32)
H3 = PFNNet( _xx, keep_prob , reuse_vars)
loss = tf.reduce_mean(tf.square(_yy - H3)) 
